...
### The system inventory — private data too

The Inventory page (format 6) is your organization's system register: system names, application URLs, data classifications, and security-posture answers (SSO/MFA, local accounts, secrets handling, internet exposure, SIEM coverage, recovery objectives). Read together, those records are a target list — "public-facing, password-only, secrets outside the vault, Strictly Confidential data" is exactly what an attacker wants ranked. Share exports therefore **exclude the entire `data.systems` section by default** (omitted, not emptied — restoring a share file leaves the receiving install's own inventory untouched), and the envelope's `systemCount` is zeroed with it so even the fleet size doesn't ride. The include-private opt-in carries the folded records, including the vendor-contact field (an external person's name/email) — treat an include-private file accordingly. Complete backups always keep the full inventory.
